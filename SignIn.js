        ...
        <SafeAreaView>
            ...
            <View style={{height: "100%", width: "100%"}}>
                ...
                <View style={{height: 52, marginHorizontal: 23, marginBottom: 25}}>
                    <TextInput onChangeText={email => setEmail(email)} defaultValue={email} activeOutlineColor="gray"
                               placeholder="Email" label="Email" mode="outlined">
                    </TextInput>
                </View>
                {check === true ?
                    <View style={{height: 52, marginHorizontal: 23}}>
                        <TextInput onChangeText={password => setPassword(password)} defaultValue={password}
                                   activeOutlineColor="gray" placeholder="Password" label="Password" mode="outlined"
                                   right={<TextInput.Icon name="eye" onPress={() => setCheck(false
                                   )}/>}>
                        </TextInput>
                    </View> :
                    <View style={{height: 52, marginHorizontal: 23}}>
                        <TextInput onChangeText={password => setPassword(password)} defaultValue={password}
                                   activeOutlineColor="gray" secureTextEntry placeholder="Password" label="Password"
                                   mode="outlined"
                                   right={<TextInput.Icon color="gray" name="eye-off" onPress={() => setCheck(true)}/>}>
                        </TextInput>
                    </View>
                }
                ...
                <View style={{
                    ...
                    <View style={{justifyContent: "flex-end"}}>
                        <TouchableOpacity onPress={() => navigation.navigate("BottomNavigation")}
                                          disabled={password.length > 8 && password.match(/\d+/g) && format.test(password) && /[a-z]/.test(password) && /[A-Z]/.test(password) ? false : true}
                                          style={{
